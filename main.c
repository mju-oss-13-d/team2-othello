...
void draw(int y, int x, const char *txt);
void drawnum(int y, int x, int n);
...
int main(int argc, char*arg[])
{
 ...
 printf("원하는 play 형태를 눌러주세요\n\n");
 printf("(0)  CPU vs CPU  \n(1)  CPU vs Player \n(2)  Player vs Player \n(3)  이전 게임을 CPU vs CPU 로 불러오기 \n(4)  이전 게임을 CPU vs Player 로 불러오기\n(5)  이전 게임을 Player vs Player 로 불러오기\n");/*choose between the type of game*/
 ...
 scanf("%d",&player);
	...
	keypad(stdscr, TRUE);
	curs_set(0); // curs_set(1); 하면 커서 보임.
	/* 끗ㅋ */
 ...
 /*the interaction between man and comp*/
	...
	if(player==1){
		draw(10, 0, "CPU goes first(1) or Man goes first(2)?");
		while(((c = getch()) != '1') && ((c = getch()) != '2')){
		}
...
CP:
  ...
  if(player==2)
  {
   ...
   if(legalmove_available(&masterboard)==1)
   {
    ...
    draw(10, 0, "                                             ");
    draw(10, 0, "This is Black's turn.");
...
next1:
     if(test_legalmove(&masterboard,the_move.x, the_move.y)==1){  /*test if it is a legal move*/
      ...
      draw(10, 0, "This move is not a legal move");
   ...
   draw(10, 0, "This is CPU Black's turn");
   if(legalmove_available(&masterboard)==1)
   {   beststrategy_entry(&masterboard,&the_move,6);
   /*the computer, or black, moves first*/
   update(&masterboard,&masterboard,the_move.x,the_move.y);
   draw(11, 0, "The move is");
   drawnum(11, 13, the_move.x);
   drawnum(11, 15, the_move.y);
   drawboard(&masterboard);}
   else
   {printf("CPU passes\n");
   passcounter+=1;
...
CPHERE:
   ...
   if(player!=0){/*when it is not AI vs AI mode*/
    if(legalmove_available(&masterboard)==1){ /*the player can make a move only if there are available cells for legal moves*/
     draw(10, 0, "                                       ");
     draw(10, 0, "This is White's turn.");
     while(legalmovemade!=1)
     {  /*this part will loop untill the player makes a legal move*/
		mousemask(BUTTON1_PRESSED, NULL);
		while ( ( c = getch()) != '.'){
...
next2:
   ...
   /*end of AI vs man*/
    ...
    /* AI vs AI mode*/
   { masterboard.turn=WHITE;
   draw(10, 0, "This is CPU WHITE's turn\n\n");
...
PP:
  ...
  if(player == 2)
  {
   ...
   {   
    ...
    while(legalmovemade!=1)
    {  /*this part will loop untill the player makes a legal move*/
		mousemask(BUTTON1_PRESSED, NULL); /*the player's move*/
		while ( ( c = getch()) != '.'){
			switch(c){
				case KEY_MOUSE:
					if (getmouse(&event) == OK){
...
next3:
     if(test_legalmove(&masterboard,the_move.x,the_move.y)==1)
     {  /*test if it is a legal move*/
      update(&masterboard,&masterboard,the_move.x,the_move.y);
      legalmovemade=1;
      drawboard(&masterboard);
     }
...
for(q=0;q<8;q++){
  ...
  case BLACK:
   draw(q, 2*p+2, "O|"); // 게임 상에서는 검은색
   break;
  case WHITE:
   draw(q, 2*p+2, "X|"); // 게임 상에서는 흰색
   break;
  case BLANK:
   if(p==0)
   {
...
draw(8, 1, " 0 1 2 3 4 5 6 7");
draw(9, 0, "Black:    White:");
drawnum(9, 6, board_draw->score_black);
drawnum(9, 16, board_draw->score_white);
 ...
 for(n=0;n<8;n++){
  if(currentboard->cells[m][n]==BLANK){
   if(test_legalmove(currentboard,m,n)==1) {     
    ...
    draw(line, 0, "Good move=");
    drawnum(line, 10, m);
    drawnum(line, 12, n);
    draw(line, 17, "The value=");
    drawnum(line, 27, value);
    line++;
    if(value>current){
